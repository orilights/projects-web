    ...
    <a :href="data.link.website" target="_blank">
      <div class="flex h-full cursor-pointer flex-col justify-between rounded-xl border bg-white p-4 transition-all duration-300 hover:shadow-md">
        ...
        <div class="mt-2 flex items-center">
          <a
            v-if="data.link?.website"
            :href="data.link.website" title="访问项目" target="_blank"
            class="p-1 text-gray-500 transition-colors hover:text-black"
          >
          ...
          <a
            ...
            :href="`https://github.com/${data.link.github}`" title="Github 仓库" target="_blank"
            class="p-1 text-gray-500 transition-colors hover:text-black"
          >
          ...
          <Transition name="fade">
            <a
              v-if="data.star"
              :href="`https://github.com/${data.link.github}`" :title="`Star ${data.star}`" target="_blank"
              class="flex items-center p-1 text-gray-500 transition-colors hover:text-black"
            >
          ...
          <Transition name="fade">
            <div
              v-if="data.lastUpdated && data.lastPushed" :title="`最后Push ${toLocaleTime(data.lastPushed)}\n最后仓库更新 ${toLocaleTime(data.lastUpdated)}`"
              class="ml-auto flex items-center rounded-full bg-slate-200 px-2 py-0.5 text-xs"
            >
              ...
              <span class="ml-1">
                {{ getDisplayTime(data.lastPushed) }}
              </span>
            </div>
          </Transition>
        </div>
